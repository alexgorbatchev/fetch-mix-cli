...
func ResolveProvider(requestedProvider, requestedModel string) (string, string, error) {
	...
	if pID == "" || pID == "auto" {
		...
		return "", "", fmt.Errorf("no active LLM API keys detected in environment. Set GEMINI_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY, or run 'fetch-mix ai' for details")
	}
...
func ExtractTracklistWithAI(ctx context.Context, providerID, modelName string, prompt string) (*GeminiResult, error) {
	...
	case "custom":
		...
		key := strings.TrimSpace(os.Getenv("OPENAI_API_KEY"))
		model = openai.Chat(modelName, openai.WithBaseURL(baseURL), openai.WithAPIKey(key))
	default:
